...
#        bert_config, model.get_pooled_output(), next_sentence_labels)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(masked_lm_example_loss, masked_lm_log_probs, masked_lm_ids,
        ...
        masked_lm_predictions =tf.compat.v1.Print(masked_lm_predictions,[masked_lm_predictions],'masked_lm_predictions')#LR tracking 4/25
        masked_lm_ids =tf.compat.v1.Print(masked_lm_ids,[masked_lm_ids],'masked_lm_ids') #LR tracking 4/25
        exp_log_prop =tf.compat.v1.Print(tf.exp(-masked_lm_example_loss),[tf.exp(-masked_lm_example_loss)],'tf.exp(-masked_lm_example_loss)') #LR tracking 4/25
#####checkun
        ...
        masked_lm_auc = tf.compat.v1.metrics.auc(labels=masked_lm_ids, predictions=tf.exp(-masked_lm_example_loss))
...
####LR 06/05/2020 added input_tensor2 and the line below to use sequence instead of pooled output
    input_tensor=tf.reduce_sum(input_tensor=input_tensor2,axis=1) 
    logits = tf.matmul(input_tensor, output_weights, transpose_b=True)
    logits = tf.nn.bias_add(logits, output_bias)
    log_probs = tf.nn.log_softmax(logits, axis=-1)
